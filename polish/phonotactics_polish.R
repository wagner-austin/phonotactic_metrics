...
polish_data <- read_csv("cleaned_response_data.csv")
polish_data[27:40] = scale(polish_data[27:40])
...
uni_bi_smoothed_model = lmer(response ~ uni_prob * bi_prob_smoothed + (1|word) + (1|subj), data=polish_data)
...
uni_bi_freq_smooth_model = lmer(response ~ uni_prob_freq_weighted * bi_prob_freq_weighted_smoothed + (1|word) + (1|subj), data=polish_data)
